...
typedef struct _MyPakcetElement{
    AVPacket *pkt;
}MyPakcetElement;
...
typedef struct _MediaState{
    ...
    uint audio_buf_size; //缓冲区大小
    int audio_buf_index;  //已经使用的数据
...
static int packet_queue_init(PacketQueue *q){
    ...
    q->pkts = av_fifo_alloc2(1, sizeof(MyPakcetElement), AV_FIFO_FLAG_AUTO_GROW);
    if(!q->pkts){
...
static int packet_queue_put_priv(PacketQueue *q, AVPacket *pkt){
    ...
    SDL_CondSignal(q->cond); //发信号通知其他线程
...
static int packet_queue_get(PacketQueue *q, AVPacket *pkt, int block){
    ...
    for (;;){
        if(av_fifo_read(q->pkts, &element, 1) >= 0){
            ...
            SDL_CondWait(q->cond, q->mutex); //队列为空 阻塞方式，等待
        }
    }
...
static void packet_queue_flush(PacketQueue *q){
    MyPakcetElement elem;
    SDL_LockMutex(q->mutex);
    while (av_fifo_read(q->pkts, &elem, 1) > 0 ){ //读取
    ...
    for(;;){
        ...
        while (ret >= 0){
            ...
            if(ret == AVERROR(EAGAIN) || ret == AVERROR_EOF){
                ...
                av_log(ms->aCtx, AV_LOG_ERROR, "Failed to recieve frame from audio decoder %s\n", av_err2str(ret));
                goto __OUT;
            }
            if(!ms->swr_ctx){
            ...
            if(ms->swr_ctx){
                ...
                int out_cout = ms->aframe->nb_samples + 512;

                int out_size = av_samples_get_buffer_size(NULL, ms->aframe->ch_layout.nb_channels, out_cout, AV_SAMPLE_FMT_S16, 0);
...
int simple_palyer2(int argc, char* argv[]){
    char* src;
    int ret = -1;
    int vidx = -1;
    int aidx = -1;
    ...
    if(ret < 0){
        av_log(NULL, AV_LOG_ERROR, "Could not ope audio codecpar: %s\n", av_err2str(ret));
        goto __END;
    }
